    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">廣播分類統計</h3>
                <div class="block-options">
                    <span class="text-muted">查看各廣播分類的觀看數據與人口統計分析</span>
                </div>
            </div>
        </div>
        ...
        <AnalyticsTabs :tabs="tabList" @tab-changed="handleTabChange">
            <!-- Tab 1: 性別總觀看人次 -->
            <template #tab-0>
                <GenderViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.radios" />
            </template>

            <!-- Tab 2: 年齡層總觀看人次 -->
            <template #tab-1>
                <AgeGroupViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.radios" />
            </template>

            <!-- Tab 3: 男性年齡層總觀看人次 -->
            <template #tab-2>
                <MaleAgeGroupViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.radios" />
            </template>

            <!-- Tab 4: 女性年齡層總觀看人次 -->
            <template #tab-3>
                <FemaleAgeGroupViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.radios" />
            </template>
        </AnalyticsTabs>
    </div>
</template>
 ...
 * - 支援日期區間篩選
 * - 使用可重用的 composable 架構（useAnalyticsTable）
 */
