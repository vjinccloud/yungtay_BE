    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">影音主分類統計</h3>
                <div class="block-options">
                    <span class="text-muted">查看影音主分類（影集、電影、短劇）的觀看數據與人口統計分析</span>
                </div>
            </div>
        </div>
        ...
        <AnalyticsTabs :tabs="tabList" @tab-changed="handleTabChange">
            <!-- Tab 1: 性別總觀看人次 -->
            <template #tab-0>
                <GenderViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.dramas.main-categories" />
            </template>

            <!-- Tab 2: 年齡層總觀看人次 -->
            <template #tab-1>
                <AgeGroupViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.dramas.main-categories" />
            </template>

            <!-- Tab 3: 男性年齡層總觀看人次 -->
            <template #tab-2>
                <MaleAgeGroupViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.dramas.main-categories" />
            </template>

            <!-- Tab 4: 女性年齡層總觀看人次 -->
            <template #tab-3>
                <FemaleAgeGroupViewsTab :categories="categories" :dateRange="dateRange" routeName="admin.analytics.dramas.main-categories" />
            </template>
        </AnalyticsTabs>
    </div>
</template>
