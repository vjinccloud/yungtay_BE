<!-- Modules/RewardActivity/Vue/Index.vue -->
<!-- 回饋活動管理 - 列表頁（含篩選 + 分頁） -->
<template>
    <div class="content">
        <BreadcrumbItem />
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title">回饋活動列表</h3>
                <div class="block-options">
                    <Link :href="route('admin.reward-activities.create')" class="btn btn-sm btn-primary">
                        ...
                        <table class="table table-bordered table-striped table-vcenter">
                            <thead>
                                <tr>
                                    ...
                                    <th>活動名稱</th>
                                    <th style="width: 120px;" class="text-center">優惠代碼</th>
                                    <th style="width: 200px;" class="text-center">活動時間</th>
                                    <th style="width: 100px;" class="text-center">獎勵數值</th>
                            ...
                            <tbody>
                                <tr v-for="item in items.data" :key="item.id">
                                    ...
                                    <td class="text-center">
                                        {{ item.reward_value }}{{ item.reward_type === 'percentage_discount' ? '%' : '' }}
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm">
                                            <Link
                                                :href="route('admin.reward-activities.edit', item.id)"
                                                class="btn"
                                                :class="item.status === 'active' ? 'btn-outline-secondary' : 'btn-outline-primary'"
                                                :title="item.status === 'active' ? '檢視' : '編輯'"
                                            >
                                                <i :class="item.status === 'active' ? 'fa fa-eye' : 'fa fa-pencil-alt'"></i>
                                            </Link>
...
export default {
    ...
    setup(props) {
        ...
        const applyFilter = () => {
            router.get(route('admin.reward-activities.index'), buildQuery(), {
                preserveState: true,
                preserveScroll: true,
            });
        };
        ...
        const resetFilter = () => {
            filterForm.keyword = '';
            filterForm.status = '';
            router.get(route('admin.reward-activities.index'), {}, {
                preserveState: true,
                preserveScroll: true,
            });
        };
        ...
        const goToPage = (page) => {
            ...
            router.get(route('admin.reward-activities.index'), buildQuery({ page }), {
                preserveState: true,
                preserveScroll: true,
            });
        };
        ...
        const conditionLabel = (type) => {
            const map = { all: '全部', order_total: '全單達到', category: '指定分類' };
        ...
        const deleteItem = (item) => {
            sweetAlert.confirm(`確定要刪除活動「${item.title}」嗎？`, () => {
                router.delete(route('admin.reward-activities.destroy', item.id), {
                    preserveScroll: true,
