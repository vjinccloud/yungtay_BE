...
{
    protected $signature = 'news-management:seed-admin-menu
                            {--parent=62 : 上層選單 ID}
                            {--force : 刪除舊選單後重新建立}';

    protected $description = '建立最新消息管理模組的後台選單';
    ...
    public function handle(): int
    {
        ...
        $startId = max($maxId + 1, 220);

        $existing = AdminMenu::where('url_name', 'admin.news-management.index')->first();
        if ($existing && !$force) {
