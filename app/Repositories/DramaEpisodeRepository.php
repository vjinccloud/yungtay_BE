...
class DramaEpisodeRepository extends BaseRepository
{
    use EpisodeRepositoryTrait;

    public function __construct(DramaEpisode $dramaEpisode)
    {
        parent::__construct($dramaEpisode);
    }
    ...
    // 保留原有的 getEpisodesByDrama 方法以維持相容性
     ...
     * @return DramaEpisode
     */
    public function saveEpisode(array $data): DramaEpisode
    {
        $id = $data['id'] ?? null;
     ...
     * @return DramaEpisode
     */
    public function save(array $data, $id = null): DramaEpisode
    {
        return DB::transaction(function () use ($data, $id) {
            ...
            // 6. 只在「新增」時自動帶 seq
            if (! $id) {
                $payload['seq'] = $this->getNextSeq($data['drama_id'], $data['season'] ?? null);
            }
     ...
     * 取得指定影音和季數的下一個序號
     *
     * @param int $dramaId 影音 ID
     * @param int|null $season 季數
     * @return int 下一個序號
     */
    public function getNextSeq($dramaId, $season = null)
    {
        $query = $this->model->newQuery();

        // 使用 Laravel 的條件查詢方法
        $query->when($dramaId === null, function($q) {
            return $q->whereNull('drama_id');
        }, function($q) use ($dramaId) {
            return $q->where('drama_id', $dramaId);
        });
    ...
    protected function moveUploadedFile($tempPath)
    {
        ...
        $finalPath = 'dramas/episodes/' . $filename;

        // 確保目錄存在
        Storage::disk('public')->makeDirectory('dramas/episodes');
    ...
    public function delete($id)
    {
        return DB::transaction(function () use ($id) {
            ...
            // 4. 撈出同「劇」與同「季」的所有 episode id，支援 drama_id/season 為 null
            $query = $this->model->newQuery();

            // drama_id 條件
            if (is_null($episode->drama_id)) {
                $query->whereNull('drama_id');
            } else {
                $query->where('drama_id', $episode->drama_id);
            }
     ...
     */
    public function paginateByDrama($dramaId, $perPage = 15, $sortColumn = 'seq', $sortDirection = 'asc', $filters = [])
    {
        $query = $this->model->newQuery()
     ...
     * 取得指定影音的影片季數
     *
     * @param int|null $dramaId 影音 ID，null 表示暫存影片
     * @return array 有影片的季數陣列
     */
    public function getVideoSeasons($dramaId)
    {
        try {
            $query = $this->model->newQuery();
            
            // 處理 drama_id 可能為 null 的情況
            if (is_null($dramaId)) {
                $query->whereNull('drama_id');
            } else {
                // 如果 dramaId 不是數字，回傳空陣列
            ...
            \Log::error("取得影音影片季數失敗 (drama_id: {$dramaId}): " . $e->getMessage());
            return [];
        }
    }
    ...
    /**
     * 檢查是否有影片資料
     * - dramaId 為 null：檢查當前管理員的暫存影片
     * - dramaId 不為 null：檢查指定影音的影片
     *
     * @param int|null $dramaId
     * @return bool
     */
    public function hasVideos($dramaId = null)
    {
        $query = $this->model->newQuery();
