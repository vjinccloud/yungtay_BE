...
namespace App\Http\Requests\DramaEpisode;

use Illuminate\Foundation\Http\FormRequest;
use App\Helpers\YouTubeHelper;

class DramaEpisodeRequest extends FormRequest
{
   /**
   ...
   public function rules(): array
   {
       ...
       return [
           // 基本欄位 - drama_id 允許為0（新增影音時的暫存狀態）
           'drama_id' => 'nullable|integer|min:0',
   ...
   public function messages(): array
   {
       return [
           // 基本欄位錯誤訊息
           'drama_id.required' => '影音ID為必填欄位',
           'drama_id.integer' => '影音ID必須為數字',
           'drama_id.min' => '影音ID不能為負數',
           'season.min' => '季數必須大於0',
           'seq.min' => '集數必須大於0',
    ...
    */
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // 自訂驗證：如果 drama_id 不為 null 且大於 0，則檢查影音是否存在
            $dramaId = $this->input('drama_id');
            if ($dramaId !== null && $dramaId > 0) {  // 修改這行：加入 !== null 檢查
                $dramaExists = \App\Models\Drama::where('id', $dramaId)->exists();
                if (!$dramaExists) {
                    $validator->errors()->add('drama_id', '指定的影音不存在');
                }
            }
